// js/main.js (Versión Completa con Sesión Persistente)
...
try {
    ...
    // ELIMINAMOS EL INICIO DE SESIÓN ANÓNIMO DE AQUÍ
...
// --- LÓGICA DE AUTENTICACIÓN MEJORADA (CON PERSISTENCIA) ---
onAuthStateChanged(auth, async (user) => {
    ...
    if (user && !user.isAnonymous) {
        ...
        console.log("Usuario anónimo encontrado.");
        ...
        console.log("Ningún usuario. Creando sesión anónima...");
        try {
            await signInAnonymously(auth);
            // onAuthStateChanged se volverá a ejecutar y caerá en el "ESTADO 2"
        } catch (error) {
...
// Manejar Logout (Simplificado)
logoutBtn.addEventListener('click', async () => {
    try {
        await signOut(auth);
        // onAuthStateChanged se encargará de crear la sesión anónima.
...
productGrid.addEventListener('click', (e) => {
    ...
    // Permitimos que usuarios anónimos agreguen al carrito.
    // El chequeo se hace al momento de "Proceder al Pago".
